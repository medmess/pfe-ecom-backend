...
public class DeliveryController : ControllerBase
{
  ...
  public async Task<ActionResult<IEnumerable<DeliveryCompanyDto>>> GetCompanies([FromQuery] string? wilaya = null)
  {
    ...
    var prices = await _context.DeliveryPrices
        .AsNoTracking()
        .Where(p => ids.Contains(p.DeliveryCompanyId))
        .ToListAsync();

    var offers = await _context.DeliveryOffers
        .AsNoTracking()
        .Where(o =>
            ids.Contains(o.DeliveryCompanyId) &&
            (o.EndsAt == null || o.EndsAt > DateTime.UtcNow))
        .ToListAsync();

    return Ok(users.Select(u =>
        ToCompanyDto(u, branches, prices, offers, wilaya)).ToList());
  }
  ...
  // ONLY DELIVERY COMPANY CAN UPDATE DELIVERY STATUS
      ...
      [FromBody] UpdateDeliveryStatusRequest request)
  {
    ...
    if (!allowedStatuses.Contains(request.DeliveryStatus))
    {
  ...
  public async Task<ActionResult<IEnumerable<DeliveryBranchDto>>> GetBranches()
  {
    ...
    var query = _context.DeliveryBranches
        .AsNoTracking()
        .AsQueryable();

    if (!User.IsInRole("Admin"))
      query = query.Where(b => b.DeliveryCompanyId == userId);
  ...
  [HttpPost("branches")]
  [Authorize(Roles = "Delivery,Admin")]
  public async Task<ActionResult<DeliveryBranchDto>> AddBranch(
      [FromBody] UpsertDeliveryBranchRequest request)
  {
    if (!ModelState.IsValid)
      return ValidationProblem(ModelState);
    ...
    return Ok(new DeliveryBranchDto
    {
      Id = branch.Id,
      Name = branch.Name,
  ...
  public async Task<ActionResult<IEnumerable<DeliveryPriceDto>>> GetPrices()
  {
    ...
    var query = _context.DeliveryPrices
        .AsNoTracking()
        .AsQueryable();

    if (!User.IsInRole("Admin"))
      query = query.Where(p => p.DeliveryCompanyId == userId);
  ...
  [HttpPost("prices")]
  [Authorize(Roles = "Delivery,Admin")]
  public async Task<ActionResult<DeliveryPriceDto>> UpsertPrice(
      [FromBody] UpsertDeliveryPriceRequest request)
  {
    if (!ModelState.IsValid)
      return ValidationProblem(ModelState);
  ...
  public async Task<ActionResult<IEnumerable<DeliveryOfferDto>>> GetOffers()
  {
    ...
    var query = _context.DeliveryOffers
        .AsNoTracking()
        .AsQueryable();

    if (!User.IsInRole("Admin"))
      query = query.Where(o => o.DeliveryCompanyId == userId);
  ...
  [HttpPost("offers")]
  [Authorize(Roles = "Delivery,Admin")]
  public async Task<ActionResult<DeliveryOfferDto>> AddOffer(
      [FromBody] UpsertDeliveryOfferRequest request)
  {
    if (!ModelState.IsValid)
      return ValidationProblem(ModelState);
  ...
  private static DeliveryCompanyDto ToCompanyDto(
      ApplicationUser user,
      List<DeliveryBranch> branches,
      List<DeliveryPrice> prices,
      List<DeliveryOffer> offers,
      string? wilaya)
  {
    var companyPrices = prices
        .Where(p => p.DeliveryCompanyId == user.Id)
        .ToList();

    var selectedPrice =
        companyPrices.FirstOrDefault(p =>
            !string.IsNullOrWhiteSpace(wilaya) &&
            p.Wilaya.Equals(wilaya, StringComparison.OrdinalIgnoreCase))
        ?? companyPrices.FirstOrDefault();

    var activeOffers = offers
        .Where(o => o.DeliveryCompanyId == user.Id)
        .Select(ToOfferDto)
        .ToList();

    var discount = activeOffers
    ...
    static decimal ApplyDiscount(decimal value, int discountPercent)
        => discountPercent <= 0
            ? value
            : Math.Max(0,
                Math.Round(value * (100 - discountPercent) / 100, 2));

    return new DeliveryCompanyDto
    {
      Id = user.Id,
      Name = !string.IsNullOrWhiteSpace(user.StoreName)
            ? user.StoreName
            : user.FullName,
      ...
      AddressPrice = ApplyDiscount(
            selectedPrice?.AddressPrice ?? 500,
            discount),

      OfficePrice = ApplyDiscount(
            selectedPrice?.OfficePrice ?? 350,
            discount),

      Branches = branches
            .Where(b => b.DeliveryCompanyId == user.Id)
            .Select(b => new DeliveryBranchDto
            {
              Id = b.Id,
              Name = b.Name,
              Wilaya = b.Wilaya,
